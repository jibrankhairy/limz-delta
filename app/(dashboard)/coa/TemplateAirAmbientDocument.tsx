...
export const TemplateAirAmbientDocument = React.forwardRef<
  HTMLDivElement,
  { data: any }
>(({ data }, ref) => {
  const {
  ...
  const visibleResults = results
    ? results.filter((param: any) => param.isVisible)
    : [];

  return (
    <div
      ref={ref}
      className="p-10 font-serif text-black bg-white relative"
      ...
      <div className="relative z-10">
          ...
          {showKanLogo && (
            <div className="flex flex-col items-end text-right">
              ...
              <div className="text-[7px] leading-tight font-sans mt-1 space-y-px">
                ...
                <p>
                  Halaman {pageNumber || 2} dari {totalPages || "..."}
                </p>
              </div>
            </div>
          )}
        ...
        <main className="text-[9px]">
          <div className="text-center my-4">
            ...
            <p className="text-xs">
              Certificate No.{" "}
              {certificateNo ? `${certificateNo}` : "DIL-AABBCCDDCOA"}
            </p>
          </div>
          ...
          <table className="w-full border-collapse border-2 border-black mb-4">
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-black p-1 text-center">
                  {sampleInfo.samplingLocation || "-"}
                </td>
                <td className="border border-black p-1 text-center">
                  Ambient Air
                </td>
                <td className="border border-black p-1 text-center">
                  {sampleInfo.samplingDate || "-"}
                </td>
                <td className="border border-black p-1 text-center">
                  {sampleInfo.samplingTime || "-"}
                </td>
                <td className="border border-black p-1 text-center">
                  {sampleInfo.samplingMethod || "-"}
                </td>
                <td className="border border-black p-1 text-center">
                  {sampleInfo.dateReceived || "-"}
                </td>
                <td className="border border-black p-1 text-center">
                  {sampleInfo.intervalTestingDate || "-"}
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table className="w-full border-collapse border-2 border-black text-[9px]">
            <thead>
              <tr className="bg-gray-200 font-bold">
                ...
                <th className="border border-black p-1 font-bold text-left">
                  Parameters
                </th>
                ...
                <th className="border border-black p-1 w-28 font-bold">
                  Regulatory Standard**
                </th>
                <th className="border border-black p-1 font-bold text-left">
                  Methods
            ...
            <tbody>
              {visibleResults.map((param: any, index: number) => (
                <tr key={`${param.name}-${index}`}>
                  ...
                  <td className="border border-black p-1 text-center">
                    {typeof param.standard === "object" &&
                    param.standard !== null
                      ? Object.values(param.standard)
                          .filter((v) => v !== null && v !== "-")
                          .join(" / ")
                      : param.standard}
                  </td>
          ...
          <table className="w-1/2 border-collapse border border-black mt-2">
            <tbody>
              ...
              <tr>
                <td className="border border-black p-1 w-1/3">Coordinate</td>
                <td className="border border-black p-1 w-px">:</td>
                <td className="border border-black p-1">
                  {sampleInfo.coordinate || "-"}
                </td>
              </tr>
              <tr>
                ...
                <td className="border border-black p-1">
                  {sampleInfo.temperature || "-"} °C
                </td>
              </tr>
              <tr>
                <td className="border border-black p-1">Pressure</td>
                <td className="border border-black p-1">:</td>
                <td className="border border-black p-1">
                  {sampleInfo.pressure || "-"} mmHg
                </td>
              </tr>
              <tr>
                ...
                <td className="border border-black p-1">:</td>
                <td className="border border-black p-1">
                  {sampleInfo.humidity || "-"} %RH
                </td>
              </tr>
              <tr>
                <td className="border border-black p-1">Wind Speed</td>
                <td className="border border-black p-1">:</td>
                <td className="border border-black p-1">
                  {sampleInfo.windSpeed || "-"} m/s
                </td>
              </tr>
              <tr>
                <td className="border border-black p-1">Wind Direction</td>
                <td className="border border-black p-1">:</td>
                <td className="border border-black p-1">
                  {sampleInfo.windDirection || "-"}
                </td>
              </tr>
              <tr>
                <td className="border border-black p-1">Weather</td>
                <td className="border border-black p-1">:</td>
                <td className="border border-black p-1">
                  {sampleInfo.weather || "-"}
                </td>
          ...
          <div className="mt-4 text-[8px]">
            ...
            <p>{sampleInfo.notes || "** Default Regulatory Note"}</p>
          </div>
        </main>
      </div>
    </div>
