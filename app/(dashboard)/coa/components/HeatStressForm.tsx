...
export function HeatStressForm({
  ...
  const handleAddRow = () => {
    const { defaultHeatStressRow } = require('../data/heatstress-data');
    const newResults = [...template.results, { id: nanoid(), ...defaultHeatStressRow }];
    onTemplateChange({ ...template, results: newResults });
  };
    ...
    <Card className="w-full max-w-6xl bg-slate-900 border-slate-800">
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>Isi Detail & Hasil Tes Heat Stress (Iklim Kerja)</CardTitle>
            ...
            {template.results.map((row: any, index: number) => (
              <div key={row.id} className="p-4 rounded-lg bg-slate-950 border border-slate-800 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div><Label>Humidity (%)</Label><Input value={row.humidity} onChange={(e) => handleChange(index, 'humidity', e.target.value)} className="mt-1 bg-slate-800"/></div>
                    <div><Label>Wet Temp (°C)</Label><Input value={row.wetTemp} onChange={(e) => handleChange(index, 'wetTemp', e.target.value)} className="mt-1 bg-slate-800"/></div>
                    <div><Label>Dew Temp (°C)</Label><Input value={row.dewTemp} onChange={(e) => handleChange(index, 'dewTemp', e.target.value)} className="mt-1 bg-slate-800"/></div>
                    <div><Label>Globe Temp (°C)</Label><Input value={row.globeTemp} onChange={(e) => handleChange(index, 'globeTemp', e.target.value)} className="mt-1 bg-slate-800"/></div>
                </div>
              </div>
            ))}
