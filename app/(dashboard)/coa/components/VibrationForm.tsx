...
import { PlusCircle, Trash2, Settings, ChevronLeft, FileSearch, Save } from "lucide-react";
import { nanoid } from "nanoid";

// Impor data untuk tombol "Tambah Baris"
import { 
...
export function VibrationForm({
  ...
  // Fungsi untuk merender baris input dinamis (bisa ditambah/hapus)
  const renderDynamicRows = () => {
    const canAddRows = ["permenaker_5", "kepmenlh_49_kejut", "kepmenlh_49_class3"].includes(template.regulation);

    return (
        <div className="space-y-4">
            {template.results.map((row, index) => (
                <div key={row.id || index} className="p-4 rounded-lg border bg-muted/30 space-y-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-4">
                        <div className="space-y-2">
                            ...
                            <Input value={row.location || ""} onChange={(e) => handleParameterChange(index, "location", e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label>Waktu</Label>
                            <Input value={row.time || ""} onChange={(e) => handleParameterChange(index, "time", e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label>Hasil Uji</Label>
                            <Input value={row.testingResult || ""} onChange={(e) => handleParameterChange(index, "testingResult", e.target.value)} />
                        </div>
                         <div className="space-y-2">
                            <Label>Unit</Label>
                            <Input value={row.unit} readOnly className="bg-muted/70 cursor-default" />
                        </div>
                    </div>
                </div>
            ))}
  ...
  // Fungsi untuk merender daftar parameter tetap (tidak bisa ditambah/hapus)
  const renderFixedParamsList = () => {
     return (
        <div className="space-y-4">
            {template.results.map((param, index) => (
                <div key={param.frequency || index} className="p-4 rounded-lg border bg-muted/30">
                     <p className="font-semibold mb-4">Frekuensi: {param.frequency} {param.unit.includes('Hz') ? '' : 'Hz'}</p>
                     <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-4">
                         <div className="space-y-2">
                            <Label>Lokasi</Label>
                            <Input value={param.location || ""} onChange={(e) => handleParameterChange(index, "location", e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label>Waktu</Label>
                            <Input value={param.time || ""} onChange={(e) => handleParameterChange(index, "time", e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label>Hasil Tes (mm/s)</Label>
                            <Input value={param.testingResult || ""} onChange={(e) => handleParameterChange(index, "testingResult", e.target.value)} />
                        </div>
                         <div className="space-y-2">
                            <Label>Metode</Label>
                            <Input value={param.method} readOnly className="bg-muted/70 cursor-default" />
                        </div>
                     </div>
                </div>
            ))}
  ...
  const renderFormContent = () => {
    switch (template.regulation) {
      // Kasus untuk regulasi dengan baris dinamis
      case "permenaker_5":
      case "kepmenlh_49_kejut":
      case "kepmenlh_49_class3":
        return renderDynamicRows();

      // Kasus untuk regulasi dengan parameter tetap
